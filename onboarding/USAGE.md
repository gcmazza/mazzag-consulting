...
- **Batch the gate.** Let PRs queue and merge them in one sitting; Team Leader is briefed to batch rather than dribble.
- **One task per Engineer session** (it's RULE-level for a reason) — short sessions verify better *and* spend less.
...
A seat can't see its own surface — which setting is on, whether it's running on your computer or in the cloud, what a permission you granted actually covers. When that comes up, it shouldn't guess: it can reach for [Dispatch](../specialists/dispatch-mobile-scout.md) — Team Leader's feature — to open your screen, with the permission you've granted, and look, then report back what's actually there. The seat that can't see its own surface reaches for the tool that can. It's read-only, it looks only where you point it, and nothing it finds becomes a change without a PR you merge.

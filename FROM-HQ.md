...
## #16 — 2026-08-21 — Your office is yours: the Factory now updates the machinery and nothing else

**What:** The manifest that decides what an update may touch — `.github/template-manifest.txt` — used to answer one question: *which upstream paths get copied?* It now answers a different one: **who owns this path after installation?** And the answer defaults to you. A path is Factory-owned (`core`) **only when we have to stay authoritative over that exact material for the framework to work correctly across every installation** — which today is seven rules: the sync engine and its two proofs (`scripts/`), the version file the update check reads, this news page (updates are delivered on it), and the four thin seat loader cards. Every one of them carries its reason next to it in the file. **Everything else the Factory ships is `office` — yours from the moment it lands.** Your seats' boot prompts, grounding, overrides and local doctrine; your `CLAUDE.md`; your rulebook; your README, onboarding, templates, tool descriptions, hosting notes, token model, mission packs, shipped skills, and your own local SECURITY and CONTRIBUTING policy. Those are seeded once if your office doesn't have them yet, then **never written again** — and when our version of one improves, the update **names** it as a *migration proposal* in the PR instead of taking yours. A broad `office` folder can hold a narrow `core` rule inside it, because **the longest matching rule wins**: that is exactly how your seats' boot material stays yours while the thin loader card that points at it keeps updating, and it is why a whole directory is never made Factory-owned just because one small thing inside it is machinery. Bare manifest lines with no owner word still mean `core`, so no older manifest breaks or silently freezes — but that is a compatibility fallback, not the policy, and CI now fails if a bare line appears in our own manifest. Two proofs ship with the engine, each paired with a negative control that must come out the other way: `scripts/sync-selftest.sh` tests the rules on fixtures, and `scripts/ownership-rehearsal.sh` builds a copy of your real office, customises twenty files across its whole office-owned surface, adds arbitrary files of its own, runs a real update against it, and checks every byte — and it runs as a **gate** before your monthly sync may touch anything. A new `update-engine` workflow runs both on every pull request that changes the engine, which is the first time the updater has been tested before it shipped rather than after.

**Why:** `seats/` was listed as a managed folder, so every file under it was overwritten on every run — including the four seats' boot prompts, grounding and overrides, the most customised material in any office. The only escape was for each office to discover the problem itself and add a `!` line per file, so **the protection existed only where somebody had already been burned.** The first attempt at this fix flipped just the paths that had already caused visible harm and left the rest Factory-owned — which quietly rebuilds the same problem one directory over, with your onboarding, templates, rulebook and README still being reclaimed. So the default moved: Core is now a list that has to be argued for, file by file, and everything else is yours.

**One thing to take from it:** a list of paths cannot tell you whether writing to one is safe — only an owner can. "Which files do we copy?" and "whose file is this?" produce the same config file and look like the same question, right up until your own words get quietly replaced by a default. And when you do draw that line, **check which way it defaults**: a boundary that protects only what has already been damaged isn't a boundary, it's a scar. If something of yours syncs, look at what happens to a file nobody thought about.

**If your factory is an older one:** open `.github/template-manifest.txt` and look for lines starting with `office`. None means your manifest predates this boundary and every managed folder is still overwritten wholesale. The update run now says so by name in its own pull request, and the fix is to copy the ownership lines across from the Factory's manifest. `versions/TEMPLATE-MANIFEST.md` explains both owners in plain words.

...
- **A recommender.** It will always tell you what it thinks the next move is — but a recommendation is input, never an order. Plans only become real when Team Leader writes them as specs and you merge them.
...
It never plans canonically, never builds, and never merges — see [`OVERRIDES.md`](OVERRIDES.md) for the full list. Lightweight is the design, not a limitation: a seat that only surfaces can't quietly become a second centre.

...
- **The executor.** Team Leader plans; Engineer builds. It works from the merged spec (chat loses to the spec when they disagree), verifies its own work before claiming anything (RULE 1), and ships complete files, never fragments (RULE 12).
...
- **The other half of the mutual audit.** Before executing a Team Leader spec, Engineer reads it critically — and is allowed to say no. A spec with a hole in it gets flagged back (RULE 11), not built around.
...
Engineer is the one seat with a **zero-input boot**. When the factory repo is attached to a Code session (in the desktop app: **Code tab → + → add your repo**), the repo's own `CLAUDE.md` loads automatically as project instructions and boots the seat. **Just start typing.** The builder is never asleep and never needs waking.

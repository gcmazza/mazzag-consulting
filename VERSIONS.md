...
| Date | Verified | By |
|---|---|---|
| 2026-07-21 | Checked all four seats' boot mechanics for live-doc drift. Code/Worker's own surface confirmed against `code.claude.com`: repo-attach auto-boot, `CLAUDE.md` load-on-attach, and the branch+PR write model all still match current docs — no update needed there. Cowork's and Designer's `GROUNDING.md` links (`support.claude.com` articles for Cowork/Design get-started) and the general `claude.com` domain **could not be reached from this Code session** — confirmed with direct `curl`, not a transient blip: the CONNECT tunnel itself returns 403, meaning this session's network policy excludes those hosts (only `code.claude.com` and the GitHub API path are open here). So the "Settings → Skills" upload step and the Design-canvas paste mechanics in `onboarding/ONBOARD-YOUR-TEAM.md` are **unverified this pass** — each seat's own step-6 "check for surface updates" at its next real boot in its own room is still the live check that matters | Robot Wrench (Code) seat |

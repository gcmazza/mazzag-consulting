# Claude in Chrome — live-page checks, a tool the team uses

**Default: OFF.** Claude in Chrome is not a seat and not a team member — it's a **tool** your Engineer seat opens for one job and stands down after. The roster is four — Coach, Team Leader, Engineer, Creative Director — and reaching for a tool never makes it five. Most factory days never need it; the day you ship something to the web and want real eyes — and hands — on the live page, it earns its keep.
...
- **No repo writes, no merges.** It changes nothing in the repo — not a file, not `main`, not a setting — and it never merges (RULE 14 binds every seat and every tool alike). What it touches is the live page in a browser, one **Allow-once**-approved action at a time; your merge button and your source are never in its reach.
...
- **No standing patrols by default.** It runs when summoned for a named check, reports, and is done. If the team wants recurring checks someday, that's a plan Team Leader writes and the human merges — not something it self-schedules.

## How its findings travel

An inspection ends with a plain report: what was checked, what was observed (RULE 3 — observed, not inferred), and what looked wrong, each finding stated without severity-softening (RULE 19). The report goes to the human, who carries it to Team Leader; anything worth remembering lands in the journal by PR. Its word is an observation, not a fix — fixes are specs, built by Engineer, gated by the human.

## How to summon it

**First use — give it permission to see.** Claude in Chrome starts with no site access: navigation is blocked on every site until you allow it in the extension's settings (Settings → Claude in Chrome → Site permissions). Allow all sites, or at least `github.com` plus the domain your live pages are served on. It's a one-time step, and until it's done every inspection fails at the first navigation — that's the permission gate working, not the tool broken.

One more surface fact worth knowing while you watch an inspection: sessions in the Chat room pause when you navigate away from them; Claude Code and Cowork-room sessions keep working while you're elsewhere. Neither is a malfunction — it's just how the surfaces behave, and knowing it saves a "did my team stop?" scare.
...
**Verify the surface empirically at first use, never assume it** — browser capabilities change, and a tool describing last season's browser is worse than none. If the live docs disagree with this page, the live docs win: open an issue so this page gets trued.

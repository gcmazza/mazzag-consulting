...
You already pay for a team — you're just using it one chat at a time. This template turns the Claude desktop app into a working factory: **four standing seats** — Coach, Team Leader, Engineer, Creative Director — that plan, build, design, and audit each other, the tools they reach for when the work leaves the repo, memory that survives every session, and **you as the only gate anything passes**. No code to understand. No command lines. You never leave the chat.
...
- **A persistent team, not a chatbot.** **Team Leader** plans and audits every change — with a security read folded into that audit whenever a PR touches credentials, workflows, auth, payments, or personal data — **Engineer** builds, **Creative Director** handles the visual lane, and **Coach** surfaces what matters and explains anything you want explained. The same four seats, every day, working the same project.
...
- **Tools the team reaches for — not more team members.** **Dispatch is Team Leader's feature**, on desktop & mobile: your orders carried to the seats, and each seat's status relayed back to your phone, so you can run the team from a waiting room or the couch. **Claude in Chrome** is a tool the team uses to open your live pages in a real browser and *drive* them — clicking and typing like a real user to walk a whole flow end to end, not just looking — always behind your per-action **Allow-once** approval. Neither is a team member and neither changes the count: the roster is four, nothing here ever merges, and nothing here touches your secrets or `main`.
...
Morning coffee: two PRs waiting. One is a page your Engineer seat built overnight against the spec you merged yesterday; Team Leader's audit is already in the thread — *safe, here's why*. You read the description (it explains itself in plain words — house style), merge from your phone, and the deploy goes out. On the drive to work you dispatch a research question; by lunch the report is back and Team Leader has folded the verified parts into tomorrow's spec. The journal wrote itself an entry. Total hands-on time: eleven minutes.
...
- **One known-good master, and everyone updates from it.** [`squidbay/factory`](https://github.com/squidbay/factory) is the template of record: every improvement lands there first, and every copy pulls from it. Your copy is severed from it on purpose — private, yours, unreachable from here — so improvements arrive by an explicit route rather than on their own. **Three ways, all ending in a pull request you merge:**
  - **Ask.** Say *"check for factory updates"* to your Engineer seat. It compares your version against the master's and opens one PR with the whole delta.
...
1. **Find your seat.** The roster is four — **Coach · Team Leader · Engineer · Creative Director** — and your boot materials are under `seats/`. Read [`seats/_shared/BOOT-COMMON.md`](seats/_shared/BOOT-COMMON.md) **first** (the boot every seat shares, and §0 is the roster), then your own `BOOT-PROMPT.md` and `OVERRIDES.md`. Your `GROUNDING.md` links point at your maker's live documentation, which beats anything written here whenever the two disagree (if they disagree, open an issue — that's how this repo stays true).
